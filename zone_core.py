...
PINE_DEFAULTS: Dict[str, Any] = {
    ...
    "legInMinBodyPct": 0.55,  # स्क्रीनशॉट के अनुसार 
...
class ZoneEngine:
    def __init__(self, df: pd.DataFrame, **kwargs):
        ...
        # Volume कॉलम की सुरक्षात्मक जाँच (Crash से बचाने के लिए)
        ...
        self.live_zones: List[Zone] = [] # नया: केवल Fresh और Tested ज़ोन्स के लिए (Speed Optimization)
    ...
    def _prepare_indicators(self):
        # Numpy Vectorization से True Range (TR) 100 गुना तेज़ी से कैलकुलेट होगा
        if self.n > 0:
            pc = np.empty(self.n)
            pc[0] = self.close[0]
            pc[1:] = self.close[:-1]
            tr = np.maximum(self.high - self.low, np.maximum(np.abs(self.high - pc), np.abs(self.low - pc)))
    ...
    def _scan_bar(self, i):
        ...
        for bCount in range(self.minBaseCount, self.maxBaseCount + 1):
            ...
            # ऑप्टिमाइज़ेशन: डुप्लीकेट ज़ोन चेक करने के लिए केवल live_zones को लूप करें (Broken ज़ोन स्किप)
            isDuplicate, checked = False, 0
            for checkZ in reversed(self.live_zones):
            ...
            self.live_zones.append(newZone)  # नया: ज़ोन को Live लिस्ट में जोड़ें

    def _update_zone_states(self, i):
        # ऑप्टिमाइज़ेशन: लूप केवल उन ज़ोन्स पर चलता है जो टूटे नहीं हैं
        ...
        for k in range(len(self.live_zones) - 1, -1, -1):
            ...
            if z.state == "Broken": 
                ...
                self.live_zones.pop(k)  # नया: ज़ोन टूटने पर उसे Live लिस्ट से हटा दें
            else: 
...
def resample_nse_session(df: pd.DataFrame, n_hours: int, session_start="09:15", session_end="15:30") -> pd.DataFrame:
    ...
    # डायनामिक कॉलम एग्रीगेशन (Dynamic Column Aggregation)
    ...
    for _, day_df in df.groupby(df.index.date):
        ...
        agg = day_df.resample(f"{n_hours}h", origin="start", label="left", closed="left").agg(agg_dict).dropna(subset=["open"])
